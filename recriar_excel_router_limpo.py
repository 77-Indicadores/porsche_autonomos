...
EXCEL = ROUTERS / "excel.py"

ROUTERS.mkdir(exist_ok=True)

# ------------------------------------------------------------
# 1. Recria app/routers/excel.py simples e funcional
# ------------------------------------------------------------
...
EXCEL.write_text(excel_code, encoding="utf-8")
print("OK - app/routers/excel.py recriado")


# ------------------------------------------------------------
# 2. Limpa main.py e registra router Excel de forma direta
# ------------------------------------------------------------

main = MAIN.read_text(encoding="utf-8")

backup = MAIN.with_suffix(".py.bak_recriar_excel")
if not backup.exists():
    backup.write_text(main, encoding="utf-8")

# Remove blocos antigos do Excel problemáticos
main = re.sub(
...
try:
    ...
    import traceback
    print("ERRO AO REGISTRAR ROUTER EXCEL")
    print(exc)
    print(traceback.format_exc())
    try:
        from app.logging_utils import log_error
        log_error("ERRO_AO_REGISTRAR_ROUTER_EXCEL", exc, {}, excel=True)
    except Exception:
        pass
'''

if "Router Excel registrado." not in main:
    main = main.rstrip() + register_block + "\n"

MAIN.write_text(main, encoding="utf-8")
print("OK - app/main.py limpo e atualizado")


# ------------------------------------------------------------
# 3. Teste local de importação
# ------------------------------------------------------------
...
print("ROTAS EXCEL:", [r for r in rotas if r.startswith("/excel")])

if not any(r.startswith("/excel") for r in rotas):
    raise SystemExit("ERRO: Router Excel ainda não apareceu nas rotas.")

print("OK FINAL - Router Excel disponível.")
